...
- Dependências pelo lockfile (`composer install` / `npm ci`), nunca `update`; não adicionar pacote sem confirmar.
- Nunca ler, logar ou colar `.env` nem segredos; usar via config, sem expor o valor.
- Testes em ambiente isolado (SQLite `:memory:`), nunca contra dev/prod.
- Em dev/teste, sem efeito colateral real — usar `Mail::fake()`, `Queue::fake()`, `Notification::fake()`, `Event::fake()`, `Http::fake()`.
- Chamada externa que cobra, envia ou altera dados de terceiros: parar e confirmar.
